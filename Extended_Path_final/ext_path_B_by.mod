...
varexo gamma_B, phi_B, R_star, fy_target_B, b_y;


%%%%%%% PARAMETERS %%%%%%%

parameters alpha, delta, beta, sigma, g_y, repl, x, omega, nu, Lambda_B;
...
ys_ini = ext_path_B_by_steadystate(zeros(M_.endo_nbr,1), ...
    [gamma_B_start; phi_B_start; R_star_start; 0; by_0], M_, options_);
ys_dem = ext_path_B_by_steadystate(zeros(M_.endo_nbr,1), ...
    [gamma_B_end;   phi_B_end;   R_star_end;   0; by_0], M_, options_);
ys_ref = ext_path_B_by_steadystate(zeros(M_.endo_nbr,1), ...
    [gamma_B_end;   phi_B_end;   R_star_end;   0; by_new], M_, options_);
...
fy_target_B_ref   = ys_ref(iff)/ys_ref(iy);   % post-reform target (b_y = 0.9)

fprintf('fy_target: start=%.4f  dem=%.4f  ref=%.4f  (salto reforma = %+.4f)\n', ...
        fy_target_B_start, fy_target_B_dem, fy_target_B_ref, ...
...
T_full = addvars(T_full, gamma_real, phi_real, R_star_real, fy_target_real, by_real, ...
    'NewVariableNames', {'gamma_B', 'phi_B', 'R_star', 'fy_target_B', 'b_y'});
...
writetable(T_full, 'output_B_by/transition_results_B_by.csv');
