...
%   e_MM = (repl * w_MM/tilde_s_m_MM + pen_bar_MM) * tilde_s_o_MM
%   pre-reform : repl = repl_0, pen_bar_MM = 0            (old rule, wage-indexed)
%   post-reform: repl = 0,      pen_bar_MM = frozen level (computed ON THE FLY at
...
varexo gamma_MM, phi_MM, R_star, fy_target_MM, repl, pen_bar_MM;


%%%%%%% PARAMETERS %%%%%%%

parameters alpha, delta, beta, sigma, b_y, g_y, x, omega, nu, Lambda_MM;
...
repl_0      = 0.40;      % pre-reform replacement rate (wage-indexed rule)
t_reform    = 55;        % onset (1970+55 = 2025); pension frozen at 2024 level
reform_span = 45;        % migration of the EXTERNAL TARGET only (cosine)
freeze_real = false;     % true: pen_bar erodes at rate x (price freeze).
...
oo_.exo_steady_state(strcmp(M_.exo_names,'pen_bar_MM')) = 0;

% exo order assumed: [gamma; phi; R_star; fy_target; repl; pen_bar]
% (confirm with disp(M_.exo_names) after first compile)
ys_ini = ext_path_MM_repl_steadystate(zeros(M_.endo_nbr,1), ...
    [gamma_MM_start; phi_MM_start; R_star_start; 0; repl_0; 0], M_, options_);
ys_dem = ext_path_MM_repl_steadystate(zeros(M_.endo_nbr,1), ...
    [gamma_MM_end;   phi_MM_end;   R_star_end;   0; repl_0; 0], M_, options_);

iy  = strcmp(M_.endo_names,'y_MM');
iff = strcmp(M_.endo_names,'f_MM');
ie  = strcmp(M_.endo_names,'e_MM');
iso = strcmp(M_.endo_names,'tilde_s_o_MM');

fy_target_MM_start = ys_ini(iff)/ys_ini(iy);
fy_target_MM_dem   = ys_dem(iff)/ys_dem(iy);   % demographic target (= baseline)
fy_target_MM_ref   = fy_target_MM_dem;          % placeholder; set at t_reform
...
id_pen         = find(strcmp(M_.exo_names, 'pen_bar_MM'));
...
for i = 1:horizon_plot
    ...
    if i == t_reform
        ...
        ys_ref = ext_path_MM_repl_steadystate(zeros(M_.endo_nbr,1), ...
            [gamma_MM_end; phi_MM_end; R_star_end; 0; 0; pen_frozen], M_, options_);
        fy_target_MM_ref = ys_ref(iff)/ys_ref(iy);
        fprintf('>>> REFORM at t=%d: pen frozen at %.5f | fy_ref=%.4f (dem %.4f, var %.4f)\n', ...
            t_reform, pen_frozen, fy_target_MM_ref, fy_target_MM_dem, ...
...
writetable(T_full, 'output_MM_repl/transition_results_MM_repl.csv');
