...
%   e_A = (repl * w_A/tilde_s_m_A + pen_bar_A) * tilde_s_o_A
%   pre-reform : repl = repl_0, pen_bar_A = 0            (old rule, wage-indexed)
%   post-reform: repl = 0,      pen_bar_A = frozen level (computed ON THE FLY at
...
varexo gamma_A, phi_A, R_star, fy_target_A, repl, pen_bar_A;


%%%%%%% PARAMETERS %%%%%%%

parameters alpha, delta, beta, sigma, b_y, g_y, x, omega, nu, Lambda_A;
...
reform_span = 5;        % migration of the EXTERNAL TARGET only (cosine)
...
oo_.exo_steady_state(strcmp(M_.exo_names,'pen_bar_A')) = 0;

% exo order assumed: [gamma; phi; R_star; fy_target; repl; pen_bar]
% (confirm with disp(M_.exo_names) after first compile)
ys_ini = ext_path_A_repl_steadystate(zeros(M_.endo_nbr,1), ...
    [gamma_A_start; phi_A_start; R_star_start; 0; repl_0; 0], M_, options_);
ys_dem = ext_path_A_repl_steadystate(zeros(M_.endo_nbr,1), ...
    [gamma_A_end;   phi_A_end;   R_star_end;   0; repl_0; 0], M_, options_);

iy  = strcmp(M_.endo_names,'y_A');
iff = strcmp(M_.endo_names,'f_A');
ie  = strcmp(M_.endo_names,'e_A');
iso = strcmp(M_.endo_names,'tilde_s_o_A');

fy_target_A_start = ys_ini(iff)/ys_ini(iy);
fy_target_A_dem   = ys_dem(iff)/ys_dem(iy);   % demographic target (= baseline)
fy_target_A_ref   = fy_target_A_dem;          % placeholder; set at t_reform
...
id_pen         = find(strcmp(M_.exo_names, 'pen_bar_A'));
...
for i = 1:horizon_plot
    ...
    if i == t_reform
        ...
        ys_ref = ext_path_A_repl_steadystate(zeros(M_.endo_nbr,1), ...
            [gamma_A_end; phi_A_end; R_star_end; 0; 0; pen_frozen], M_, options_);
        fy_target_A_ref = ys_ref(iff)/ys_ref(iy);
        fprintf('>>> REFORM at t=%d: pen frozen at %.5f | fy_ref=%.4f (dem %.4f, var %.4f)\n', ...
            t_reform, pen_frozen, fy_target_A_ref, fy_target_A_dem, ...
...
writetable(T_full, 'output_A_repl/transition_results_A_repl.csv');
